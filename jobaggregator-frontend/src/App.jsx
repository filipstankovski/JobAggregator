...
const S = {
  ...
  logo: { fontFamily: "'Playfair Display', serif", fontWeight: 900, fontSize: '1.4rem', letterSpacing: '-0.02em', color: 'var(--text)' },
  logoAccent: { color: 'var(--accent)' },
  headerBadge: { fontFamily: "'DM Mono', monospace", fontSize: '0.75rem', color: 'var(--muted)', background: 'var(--border)', padding: '3px 12px', borderRadius: 99 },
  ...
  heroTitle: { fontFamily: "'Playfair Display', serif", fontWeight: 900, fontSize: 'clamp(2.2rem,5vw,3.8rem)', lineHeight: 1.05, letterSpacing: '-0.03em', maxWidth: 640 },
  heroSub: { marginTop: '0.75rem', color: 'var(--muted)', fontWeight: 300, maxWidth: 480 },
  ...
  label: { fontFamily: "'DM Mono', monospace", fontSize: '0.68rem', fontWeight: 500, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--muted)' },
  input: { background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 8, color: 'var(--text)', fontFamily: "'DM Sans', sans-serif", fontSize: '0.9rem', padding: '0.6rem 0.9rem', outline: 'none', transition: 'border-color 0.15s', width: '100%', appearance: 'none', WebkitAppearance: 'none' },
  ...
  sourceLabel: { fontFamily: "'DM Mono', monospace", fontSize: '0.68rem', letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--muted)' },
  pill: { fontSize: '0.78rem', padding: '4px 14px', borderRadius: 99, border: '1px solid var(--border)', cursor: 'pointer', background: 'transparent', color: 'var(--muted)', fontFamily: "'DM Sans', sans-serif", transition: 'all 0.15s' },
  pillActive: { background: 'rgba(200,55,45,0.12)', borderColor: 'var(--accent)', color: 'var(--accent)', fontWeight: 500 },
  searchBtn: { background: 'var(--accent)', color: '#fff', border: 'none', borderRadius: 8, fontFamily: "'DM Sans', sans-serif", fontSize: '0.9rem', fontWeight: 500, padding: '0.65rem 1.8rem', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '0.5rem', transition: 'opacity 0.15s' },
  resetBtn: { background: 'transparent', color: 'var(--muted)', border: '1px solid var(--border)', borderRadius: 8, fontFamily: "'DM Sans', sans-serif", fontSize: '0.9rem', padding: '0.65rem 1.2rem', cursor: 'pointer', transition: 'all 0.15s' },
  results: { maxWidth: 1200, margin: '0 auto', padding: '0 clamp(1rem,4vw,3rem) 4rem' },
  resultsMeta: { fontSize: '0.82rem', color: 'var(--muted)', marginBottom: '1.25rem', fontFamily: "'DM Mono', monospace" },
  grid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(360px, 1fr))', gap: '1rem' },
  card: { background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: '1.4rem', display: 'flex', flexDirection: 'column', gap: '0.6rem', boxShadow: 'var(--shadow)', transition: 'transform 0.15s, box-shadow 0.15s' },
  cardSource: { fontFamily: "'DM Mono', monospace", fontSize: '0.68rem', letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--accent)', fontWeight: 500 },
  cardTitle: { fontFamily: "'Playfair Display', serif", fontWeight: 700, fontSize: '1.05rem', lineHeight: 1.3, color: 'var(--text)' },
  cardCompany: { fontSize: '0.85rem', color: 'var(--muted)', display: 'flex', alignItems: 'center', gap: '0.4rem' },
  cardDesc: { fontSize: '0.85rem', lineHeight: 1.65, display: '-webkit-box', WebkitLineClamp: 3, WebkitBoxOrient: 'vertical', overflow: 'hidden' },
  cardMeta: { display: 'flex', flexWrap: 'wrap', gap: '0.4rem', marginTop: '0.25rem' },
  tag: { fontSize: '0.74rem', padding: '3px 10px', borderRadius: 99, border: '1px solid var(--border)', color: 'var(--muted)', display: 'flex', alignItems: 'center', gap: '0.3rem', background: 'var(--bg)' },
  ...
  applyBtn: { display: 'inline-flex', alignItems: 'center', gap: '0.4rem', background: 'var(--text)', color: 'var(--bg)', padding: '0.55rem 1.1rem', borderRadius: 8, fontSize: '0.82rem', fontWeight: 500, textDecoration: 'none', transition: 'opacity 0.15s' },
  stateBox: { textAlign: 'center', padding: '5rem 2rem', color: 'var(--muted)' },
  stateBig: { fontSize: '3rem', marginBottom: '0.75rem' },
  stateText: { fontSize: '1rem', fontWeight: 300 },
}
          ...
          <>
            ...
            <div style={S.grid}>
              {filteredJobs.map((job, i) => (
                <div key={job.id} style={{ animation: 'fadeUp 0.3s ease both', animationDelay: `${Math.min(i * 25, 400)}ms` }}>
                  <JobCard job={job} />
                </div>
              ))}
            </div>
